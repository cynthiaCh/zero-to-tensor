# Zero to Tensor

从数学直觉到 AI 张量的学习实验室。

这个项目记录我如何通过抄写、追问、顿悟、代码实验和复盘，一步步理解数学与 AI。它不是一套已经完成的教程，而是一份持续生长的学习档案。

## 在线阅读

GitHub Pages 可以直接使用 `docs/` 目录发布：

- 首页：`docs/index.md`
- 文章：`docs/notes/`
- 图片：`docs/assets/images/`

启用方式：
.venv/bin/python
1. 打开 GitHub 仓库的 `Settings`
2. 进入 `Pages`
3. Source 选择 `Deploy from a branch`
4. Branch 选择 `main`，目录选择 `/docs`

## 项目结构

```text
zero-to-tensor/
├── README.md
├── docs/
│   ├── index.md
│   ├── notes/
│   │   └── sqrt2.md
│   └── assets/
│       └── images/
├── experiments/
│   ├── assets/
│   │   └── resnet/
│   ├── browser_sentiment/
│   ├── explain_resnet_layers.py
│   ├── run_resnet_pytorch.py
│   ├── tanh.py
│   ├── train_resnet_custom.py
│   └── sqrt2_triangle.py
├── notebooks/
├── questions/
│   └── unsolved.md
└── sparks/
    └── sudden-insights.md
```

## 记录方法

每个概念尽量经过六步：

1. 原文抄写：只抄真正卡住我或打动我的句子。
2. 我没懂什么：写下最真实的问题。
3. 我突然通了什么：记录顿悟瞬间。
4. 我用自己的话重说一遍：讲给昨天的自己听。
5. 和 AI 的连接：把数学直觉接到 AI 概念上。
6. 一句话自测：如果一句话讲不清，就继续追问。

## 当前内容

- [sqrt(2)：一个写不完的长度](docs/notes/sqrt2.md)
- [指数分布：为什么“第 1 分钟”看起来最危险](docs/notes/exponential-waiting-time.md)
- ResNet 图像分类推理与层结构解释：`experiments/run_resnet_pytorch.py`、`experiments/explain_resnet_layers.py`
- ResNet 自定义 ImageFolder 数据集训练：`experiments/train_resnet_custom.py`
- 浏览器情感分析历史草稿：`experiments/browser_sentiment/`
- RNN 按时间步前向传播手算演示：`experiments/rnn_step_by_step.py`
- Transformer Attention 加权平均手算演示：`experiments/attention_weighted_average.py`
- Transformer Q/K/V Self-Attention 演示：`experiments/qkv_self_attention.py`
- Transformer Two-Head Multi-Head Attention 演示：`experiments/multi_head_attention.py`
- Auto-Regressive 逐 token 生成演示：`experiments/autoregressive_generation.py`
- Transformer position-wise Feed-Forward 演示：`experiments/transformer_feed_forward.py`
